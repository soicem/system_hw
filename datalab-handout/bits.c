...
int bitCount(int x) {
  // 0x11 == 0b10001
  int mask = 0x11 + (0x11 << 8) + (0x11 << 16) + (0x11 << 24);// amazing!
  int count = x & mask;
  int result = 0;
  int singleMask = 0x0f;
  x = x >> 1;
  ...
  x = x >> 1;
  count += x & mask;
  result += (count & singleMask) + ((count >> 4) & singleMask)
                 + ((count >> 8) & singleMask) + ((count >> 12) & singleMask)
                 + ((count >> 16) & singleMask) + ((count >> 20) & singleMask)
                 + ((count >> 24) & singleMask) + ((count >> 28) & singleMask);
  // last op is for one bit
  return result;
}
/*
